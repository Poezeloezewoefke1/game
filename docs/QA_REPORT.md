...
| 83 | **The fix for defect 75 moved a pillar off one objective and onto another** | Hallow, once its guard moved to the cave: `press.blocked  the crystal_cave: the host's line of sight (-21.0, 1.5, -37.6) -> (-23.0, 0.9, -39.8) is blocked by @StaticBody3D@864 at (-22.1, 1.2, -38.8)` - the crystal could not be taken | The blocker is `D03_ruin_pillar`, **the same prop that was moved to fix defect 75**. It went from 1.0 m off the coupling socket to 1.71 m off the cave crystal, where it sat harmlessly for as long as players approached that crystal from the other side. Rotating Hallow's locks put the guard fight there, the approach changed, and the host's ray was blocked from every angle a player now arrives at. Measuring every level settled how widespread it is: three interactables sit under 2.5 m from a solid prop - Cinder's cave crystal at 1.71 m (latent, same pillar), Hallow's at 1.71 m (this defect), and Hallow's coupling socket at 1.38 m, which I had placed there myself an hour earlier without checking. Everything else clears 4.0 m, and Nerava has no solid dressing at all - which is exactly why this class has never appeared there. All three moved, and `test_scene_integrity` now requires 2.5 m of room around every interactable. That rule is the generalisation of defects 55, 56, 57, 75 and this one; each of those was fixed by moving one prop and none of them added a rule, which is how the next prop was free to land in the same place |
| 84 | **The campaign was one planet long. Cinder and Hallow could not be reached by playing.** | A new test phase that walks the only route to the second planet, printing where it is at each step: `lobby ... completed=["nerava"]` then `ship ... completed=[]`. The win survives the lobby and is thrown away by starting the next session | The end screen offers Retry, Lobby and Quit - there is no "fly on" - so winning and going on means lobby, then start again, and `host_start_session` rebuilt the snapshot with `fresh_ship_snapshot(session_epoch)`, dropping `completed_missions`. Its comment said so plainly: "a brand-new session starts the campaign over". That is right for a brand-new session and wrong for the one path a crew actually takes. Meanwhile `_host_reset_facts`, two hundred lines below, had been carefully carrying `completed_missions` through the lobby *for this exact purpose* - two pieces of code with opposite intentions and no test between them. `fresh_ship_snapshot` already accepted the argument; the call simply never passed it. **Why nothing caught it:** every automated test ends at extraction, and every playtest of Cinder and Hallow used the driver's `--mission=` override - a flag added so the harness COULD fly the other planets, which had the side effect of proving they worked while hiding that a player could not get to them. The nav console would have said so on any second flight; nothing ever took one |
...
| I30 | A test measured elapsed time by adding up frame deltas | `test_lan_discovery :: expiry` failed three times in one session, always while other work was running, always passing on the immediate re-run | Godot CLAMPS the delta it reports when a frame runs long, so a sum of deltas drifts behind the wall clock under load - and the assertion is about the wall clock: "it stays listed for roughly the configured timeout". Each failure cost the trouble of proving it had nothing to do with the change in hand, twice for player movement and once for level geometry, neither of which can reach a UDP socket. It reads `Time.get_ticks_msec()` now. Tolerating it as a known flake was the wrong call and is withdrawn: a test that measures the wrong thing spends attention and returns noise  **Amended:** it failed again after that fix, in the opposite direction - the entry dropped out EARLY. The clamped delta was real and was one of two causes. The other is that an entry ages from its last PACKET, not from the moment the host stopped announcing, so a stop landing just before the next announcement leaves the entry already a full interval old. The lower bound was a hard-coded 1.0 s, which is exactly the announce interval: zero margin, guaranteed to fire eventually. It is derived from the two constants now, with slack, and says the numbers it used when it fails. Recording this because the first entry read as though the matter were closed, and a fix that removes one of two causes and then reports the flake solved is worse than one that says which half it addressed  **Amended again (third).** Two further failures, "waited 0.3s" then "waited 0.6s", neither of which was a margin. The test inherited its subject from the flood test that runs before it and injects 64 packets against a 32-entry cap: the first time the entry was gone outright, the second it was present but roughly three seconds STALE, so stopping the announcer left it 0.6 s of life. It now establishes the session itself and then waits for a fresh announcement before starting the clock, because present and fresh are not the same thing when what you are timing is age. Three separate causes behind one assertion, each exposed by fixing the last: a clamped delta, a bound with no margin, and an inherited precondition |
...
| I41 | **An incomplete fix to a hard-coded crystal, two lines below the comment warning about it** | Hallow, immediately after its locks were rotated: `the coupling socket: five presses while aimed at it did not take`, with `press.blocked ... the host's line of sight is CLEAR - the refusal is something else` and no host rejection logged at all | The coupling errand asked "is CRYSTAL_CAVE unlocked yet?" as its success condition. Rotating Hallow's locks made the coupling open the GROVE, so the presses all landed, the grove unsealed, and the driver went on watching a crystal that is behind a guard there and never opens. What makes this one worth its own row is that the same hard-coding had just been found and fixed at the call site eight lines above, with a comment explaining precisely this failure mode - and the predicate below it was missed. Finding a class of bug, writing down what it does, and then fixing one instance of it is how the second instance gets to look like a game defect. Both now take the crystal id from the mission's own lock table |
...
**A flood of discovery packets leaves a real session roughly three seconds
stale.** Measured while making the expiry test self-contained: after the flood
test injects 64 announcements against the browser's 32-entry cap, the legitimate
session's entry has a `last_seen` about three seconds old, against an announce
interval of one second. It recovers, and the cap does its job - the browser
never grows past 32 and an injected announcement is still attributed to its real
sender - so this is not an exploit, and the test is now robust to it either way.

What has NOT been established is which end is starved: whether the listener is
behind on draining, or the announcer's own tick is being delayed while it
happens. The two are distinguishable with a timestamped log at each end, and
that is what to do before anyone changes the drain loop. Noting it because a
three-second gap on a one-second interval is a large ratio, and because the
browser's cap policy rejects unknown keys when full - so a session that lapses
while an attacker holds all 32 slots would have no way back.
