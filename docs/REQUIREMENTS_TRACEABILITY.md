...
| MIS-033 | **The campaign can be played past its first planet** - a finished mission unlocks the next, the progress survives the lobby and the next session, and the crew can plot and fly the new course | `GameManager.host_start_session` carries `completed_missions`; `nav_console.next_id` | `test_mission_flow :: handover`, which flies it | AUTO |
