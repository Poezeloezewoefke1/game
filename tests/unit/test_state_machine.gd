...
func test_happy_path_edges_are_legal() -> void:
	var path := [
		MS.LOBBY_READY, MS.TRANSITIONING_TO_SHIP, MS.SHIP_IDLE,
		MS.LAUNCHING, MS.IN_TRANSIT, MS.LANDING,
		MS.TRANSITIONING_TO_SURFACE, MS.FIND_TEMPLE, MS.FIND_CRYSTALS,
		MS.ACTIVATE_ALTAR, MS.RETRIEVE_STAR_MAP, MS.BOSS_FIGHT,
		MS.RETURN_TO_DROP_POD,
...
func test_cannot_skip_the_mission() -> void:
	check_false(MissionRules.is_valid_transition(MS.SHIP_IDLE, MS.MISSION_COMPLETE),
...
func test_failure_is_reachable_from_every_nerava_state() -> void:
	for state in MissionRules.SURFACE_STATES:
...
func test_failure_is_not_reachable_off_nerava() -> void:
	...
	check_false(MissionRules.is_valid_transition(MS.SHIP_IDLE, MS.MISSION_FAILED),
		"the hub cannot fail the mission")


func test_retry_and_lobby_exits() -> void:
	check(MissionRules.is_valid_transition(MS.MISSION_FAILED, MS.TRANSITIONING_TO_SURFACE),
		"failure can retry")
	check(MissionRules.is_valid_transition(MS.MISSION_FAILED, MS.RETURNING_TO_LOBBY),
		"failure can return to the lobby")
	check_false(MissionRules.is_valid_transition(MS.MISSION_COMPLETE, MS.TRANSITIONING_TO_SURFACE),
...
func test_required_objective_wording() -> void:
	...
	check_eq(MissionRules.objective_text(MS.SHIP_IDLE),
		"Ready the ship for launch.", "ship objective wording")
	check_eq(MissionRules.objective_text(MS.BOSS_FIGHT),
		"Kill the Warden.", "boss objective wording")
